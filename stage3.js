    ...
    function PuzzleAdamBasket({ coins, onSpendCoins, onBack, onSolved }) {
      ...
      const handlePointerMove = useCallback((clientX) => {
        ...
        if (phaseRef.current !== 'playing') return; // 모달 표시 중에는 이동 차단
      ...
      useEffect(() => {
        ...
        if (!el) return;
        const onTouchMove  = (e) => {
          if (phaseRef.current !== 'playing') return; // playing 상태일 때만 preventDefault
          e.preventDefault();
          handlePointerMove(e.touches[0].clientX);
        };
        const onTouchStart = (e) => {
          if (phaseRef.current !== 'playing') return; // 모달 버튼 터치가 막히지 않도록
          e.preventDefault();
          handlePointerMove(e.touches[0].clientX);
        };
      ...
      return (
        <div className="flex-1 w-full h-full relative overflow-hidden select-none">
          ...
          {/* Level Clear Modal - gameAreaRef 영역 외부에서 렌더링되므로 이벤트 충돌 원천 방지 */}
          {phase === 'levelClear' && (
            <div className="absolute inset-0 bg-black/70 z-30 flex items-center justify-center p-4">
              <div className="bg-[#fdfaf2] border-4 border-[#8b5a2b] p-5 rounded-2xl shadow-2xl max-w-[260px] w-full text-center scale-in font-layton-myeongjo">
                ...
                <button
                  ...
                  className="w-full py-2 bg-gradient-to-b from-[#e7b85e] to-[#ab7418] text-[#2c1a0c] font-black text-xs rounded-xl border-2 border-[#ffe8b5] shadow active:scale-95 cursor-pointer"
                >
                  다음 단계 도전 ➡️
                </button>
              </div>
          ...
          {phase === 'gameOver' && (
            <div className="absolute inset-0 bg-black/80 z-30 flex items-center justify-center p-4">
              <div className="bg-[#fdfaf2] border-4 border-red-800 p-5 rounded-2xl shadow-2xl max-w-[260px] w-full text-center scale-in font-layton-myeongjo">
                ...
                <button
                  onClick={restartGame}
                  className="w-full py-2 bg-gradient-to-b from-red-600 to-red-800 text-white font-black text-xs rounded-xl border-2 border-red-400 shadow active:scale-95 cursor-pointer"
                >
                  다시 도전 🔄
                </button>
              </div>
            ...
            <div className="absolute inset-0 bg-[#e0f7e9] z-30 flex flex-col items-center justify-center p-5 overflow-auto">
              ...
              <div className="relative z-10 bg-[#fdfaf2] border-4 border-[#8b5a2b] p-5 rounded-2xl shadow-[0_15px_40px_rgba(0,0,0,0.3)] max-w-[290px] w-full text-center font-layton-myeongjo">
                ...
                <button
                  ...
                  className="w-full py-2 bg-gradient-to-b from-[#e7b85e] to-[#ab7418] text-[#2c1a0c] font-black text-xs rounded-xl border-2 border-[#ffe8b5] shadow-md hover:scale-[1.02] active:scale-95 transition-all cursor-pointer"
                >
                  다음 스테이지로 ➡️
                </button>
              </div>
